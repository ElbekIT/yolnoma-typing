...
export const OwnerAboutView: React.FC<OwnerAboutViewProps> = ({
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-8 pb-12">
      ...
      {/* 3. DIRECT TELEGRAM & PHONE CONTACT SECTION */}
      <section className="rounded-3xl bg-[var(--card-bg)] border border-[var(--sub-alt)] p-6 sm:p-8 lg:p-10 shadow-sm">
        <div className="max-w-4xl mx-auto space-y-6 text-center">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[var(--main-color)]/10 text-[var(--main-color)] text-xs font-black uppercase tracking-wider">
            <Send className="w-3.5 h-3.5" />
            <span>To'g'ridan-to'g'ri Aloqa & Hamkorlik</span>
          </div>

          <h3 className="text-2xl sm:text-3xl font-black text-[var(--text-color)] tracking-tight">
            Muallif va Dasturchi bilan Bog'lanish
          </h3>

          <p className="text-sm sm:text-base text-[var(--sub-color)] max-w-2xl mx-auto leading-relaxed">
            Yolnoma platformasi, takliflar, yangi loyihalar yoki hamkorlik bo'yicha <strong className="text-[var(--text-color)]">Elbek Qoriyev</strong> ga to'g'ridan-to'g'ri Telegram yoki telefon orqali bemalol yozishingiz va aloqaga chiqishingiz mumkin!
          </p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto pt-2">
            {/* Primary Telegram Link Button */}
            <a
              href="https://t.me/elbekdesign_va_webdasturchi_uz"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-3 p-4 rounded-2xl bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white font-extrabold text-sm shadow-xl shadow-sky-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
            >
              <Send className="w-5 h-5" />
              <div className="text-left">
                <div className="text-[10px] uppercase font-bold opacity-80">Telegram orqali yozish</div>
                <div className="text-xs sm:text-sm font-black">@elbekdesign_va_webdasturchi_uz</div>
              </div>
            ...
            <a
              href={`tel:${phoneNumber}`}
              className="flex items-center justify-center gap-3 p-4 rounded-2xl bg-[var(--sub-alt)] hover:bg-[var(--main-color)] hover:text-white border border-[var(--sub-color)]/20 text-[var(--text-color)] font-extrabold text-sm transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
              <PhoneCall className="w-5 h-5 text-[var(--main-color)] group-hover:text-white" />
              <div className="text-left">
                <div className="text-[10px] uppercase font-bold opacity-80 text-[var(--sub-color)]">To'g'ridan-to'g'ri qo'ng'iroq</div>
                <div className="text-xs sm:text-sm font-black">{formattedPhone}</div>
              </div>
              <ExternalLink className="w-4 h-4 ml-auto opacity-75" />
            </a>
          </div>

          <div className="pt-2 text-xs text-[var(--sub-color)] flex items-center justify-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-500" />
            <span>Xabarlarga odatda 15-30 daqiqa ichida javob beriladi.</span>
          </div>
        </div>
      </section>
