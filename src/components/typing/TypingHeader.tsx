...
export const TypingHeader: React.FC<TypingHeaderProps> = ({
  ...
  return (
    <>
      <div
        className={`w-full max-w-4xl mx-auto mb-4 flex items-center justify-center transition-all duration-300 transform ${
          isTestActive
            ? 'opacity-0 pointer-events-none -translate-y-4 scale-95 h-0 overflow-hidden mb-0'
            : 'opacity-100 translate-y-0 scale-100'
        }`}
      >
        {/* Unified Monkeytype Control Bar */}
        <div className="w-full bg-[var(--card-bg)]/90 backdrop-blur-md border border-[var(--sub-alt)] rounded-2xl p-2.5 shadow-lg flex flex-wrap items-center justify-between gap-3 text-xs font-semibold">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[var(--sub-alt)] text-[var(--text-color)] hover:border-[var(--main-color)] border border-[var(--sub-color)]/20 hover:bg-[var(--sub-alt)]/80 transition-all font-bold group"
          ...
          {/* Mode Selector (So'zlar, Jumlalar, etc.) */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-[var(--card-bg)] border border-[var(--sub-alt)] rounded-3xl shadow-2xl p-6 z-50 text-xs space-y-4 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-[var(--sub-alt)]">
              ...
              <button
                onClick={() => setShowLangModal(false)}
                className="p-2 rounded-xl bg-[var(--sub-alt)] text-[var(--sub-color)] hover:text-[var(--text-color)] hover:bg-[var(--sub-alt)]/80 transition-all"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
                ...
                filteredLanguages.map((l) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3.5 py-3 rounded-2xl transition-all font-bold ${
                        isSelected
