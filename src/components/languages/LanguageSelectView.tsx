...
import { Search, Globe, Check, ArrowRight, ArrowLeft, Sparkles, BookOpen, ChevronLeft, ChevronRight, Filter } from 'lucide-react';
...
type LanguageCategory = 'all' | 'turkic' | 'european' | 'asian_east' | 'cyrillic' | 'rtl';
...
export const LanguageSelectView: React.FC<LanguageSelectViewProps> = ({ onConfirm, onCancel }) => {
  ...
  const getLanguageCategory = (l: LanguageInfo): LanguageCategory[] => {
    ...
    // Turkic languages
    if (['uz-latn', 'uz-cyrl', 'tr', 'kk', 'ky', 'tk', 'az'].includes(code)) {
      cats.push('turkic');
    }
    // European languages
    if (['en', 'es', 'de', 'fr', 'it', 'pt', 'nl', 'pl', 'cs', 'sk', 'ro', 'hu', 'sv', 'no', 'da', 'fi', 'el', 'hr', 'la'].includes(code)) {
      cats.push('european');
    }
    // Asian & Eastern languages
    if (['zh-hans', 'ja', 'ko', 'hi', 'bn', 'ta', 'te', 'th', 'vi', 'id', 'ms', 'ar', 'fa', 'ur', 'he', 'ka', 'hy', 'mn'].includes(code)) {
      cats.push('asian_east');
    }
    // Cyrillic script
    if (l.script === 'Cyrillic' || ['uz-cyrl', 'ru', 'kk', 'ky', 'tg', 'uk', 'be', 'bg', 'sr', 'mn'].includes(code)) {
      cats.push('cyrillic');
    }
    // RTL
    if (l.dir === 'rtl' || ['ar', 'fa', 'ur', 'he'].includes(code)) {
      cats.push('rtl');
    }
  ...
  // Total pages
  const totalPages = Math.max(1, Math.ceil(filteredLanguages.length / ITEMS_PER_PAGE));

  // Current page items (exactly 10 per page)
  const paginatedLanguages = useMemo(() => {
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    return filteredLanguages.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  ...
  const categories = [
    ...
    { id: 'asian_east' as LanguageCategory, label: 'Osiyo & Sharq', count: allLanguages.filter(l => getLanguageCategory(l).includes('asian_east')).length },
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs px-2 py-0.5 rounded-full bg-[var(--main-color)]/10 text-[var(--main-color)] font-mono font-bold">
                {allLanguages.length} ta til
              </span>
            </div>
            <p className="text-xs text-[var(--sub-color)] mt-0.5">
              Tez yozish mashqlari va testlar uchun o'zingizga qulay tilni tanlang (sahifada 10 tadan ko'rsatiladi)
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="relative flex items-center bg-[var(--card-bg)] border border-[var(--sub-alt)] rounded-xl px-3.5 py-2.5 shadow-sm focus-within:border-[var(--main-color)] transition-colors">
            ...
            <input
              ref={searchInputRef}
              type="text"
              placeholder="Til nomi, mamlakat yoki kodi bo'yicha qidirish (masalan: o'zbekcha, english, ruscha, turkcha)..."
              value={searchQuery}
          ...
          {/* Languages Cards Grid (10 items per page) */}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      {/* Number Index */}
                      <span className="text-[11px] font-mono text-[var(--sub-color)]/60 w-5 shrink-0 text-right">
                        {globalIndex}.
                      </span>
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {l.dir === 'rtl' && (
                            <span className="text-[9px] px-1 py-0.2 rounded bg-amber-500/10 text-amber-500 font-mono shrink-0">
                              RTL
                            </span>
                          )}
                        </div>
          ...
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 border-t border-[var(--sub-alt)]">
              <div className="text-xs text-[var(--sub-color)] font-mono">
                Ko'rsatilmoqda: <span className="text-[var(--text-color)] font-bold">{(currentPage - 1) * ITEMS_PER_PAGE + 1} - {Math.min(currentPage * ITEMS_PER_PAGE, filteredLanguages.length)}</span> / jami <span className="text-[var(--text-color)] font-bold">{filteredLanguages.length}</span> ta til
              </div>
        ...
        <div className="space-y-4">
          <div className="bg-[var(--card-bg)] border border-[var(--sub-alt)] rounded-2xl p-5 shadow-sm space-y-4 sticky top-4">
            ...
            <div className="p-3 rounded-xl bg-[var(--bg-color)] border border-[var(--sub-alt)] space-y-1.5">
              ...
              <p className="text-xs text-[var(--text-color)]/80 italic font-mono leading-relaxed line-clamp-3" dir={activeLangObj.dir || 'ltr'}>
