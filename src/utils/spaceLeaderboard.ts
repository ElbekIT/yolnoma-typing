...
// Boshlang'ich kosmik chempionlar (baza bo'sh bo'lgan holatda namoyish uchun)
const DEFAULT_SPACE_HEROES: SpaceScoreRecord[] = [
  {
    score: 18450,
    wave: 14,
    wpm: 92,
    ...
    maxStreak: 54,
    language: 'uz',
    playerName: 'Sardor_Typist',
    playerAvatar: 'https://api.dicebear.com/7.x/bottts/svg?seed=Sardor',
    uid: 'hero_1',
  ...
  },
  {
    ...
    maxStreak: 41,
    language: 'uz',
    playerName: 'Nodira_Speed',
    playerAvatar: 'https://api.dicebear.com/7.x/bottts/svg?seed=Nodira',
    uid: 'hero_2',
  ...
  },
  {
    ...
    maxStreak: 33,
    language: 'uz',
    playerName: 'Koinot_Qahramoni',
    playerAvatar: 'https://api.dicebear.com/7.x/bottts/svg?seed=Cosmo',
    uid: 'hero_3',
  ...
  },
  {
    ...
    maxStreak: 28,
    language: 'en',
    playerName: 'Cyber_Pilot',
    playerAvatar: 'https://api.dicebear.com/7.x/bottts/svg?seed=Pilot',
    uid: 'hero_4',
  ...
  },
  {
    ...
    maxStreak: 22,
    language: 'uz',
    playerName: 'Yolnoma_Racer',
    playerAvatar: 'https://api.dicebear.com/7.x/bottts/svg?seed=Racer',
    uid: 'hero_5',
 ...
 */
export async function getTopSpaceScores(limitCount: number = 10): Promise<SpaceScoreRecord[]> {
  ...
  try {
    ...
    localScores.forEach((ls) => {
      if (!scores.some((s) => s.uid === ls.uid && s.score === ls.score && s.createdAt === ls.createdAt)) {
        scores.push(ls);
      }
    });
  } catch {}

  // 4. Agar umumiy natijalar kam bo'lsa, default chempionlarni aralashtirib berish
  if (scores.length < 5) {
    DEFAULT_SPACE_HEROES.forEach((dh) => {
      if (!scores.some((s) => s.playerName === dh.playerName && s.score === dh.score)) {
        scores.push(dh);
      }
    });
  }
