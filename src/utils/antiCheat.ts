...
class AntiCheatSystem {
   ...
   * Enforces e.isTrusted, delta >= 15ms threshold, and max 280 WPM limit.
   */
  public registerKeystroke(e: ReactKeyboardEvent<HTMLInputElement> | KeyboardEvent, typedLength: number): boolean {
    ...
    // 2. Measure delay between physical keystrokes (Keystroke Dynamics)
    if (this.keyTimes.length > 0) {
      const lastTime = this.keyTimes[this.keyTimes.length - 1];
      const delta = now - lastTime;

      // Inhumanly fast input (delta < 15ms)
      if (delta < 15) {
        this.fastDeltaCounter++;
        // If 2 or more consecutive keys arrive in < 15ms or instant macro burst (< 3ms)
        if (this.fastDeltaCounter >= 2 || delta < 3) {
          this.isFlagged = true;
          this.flagReason = "G'ayritabiiy robotik tezlik (harflar orasidagi vaqt < 15ms) aniqlandi va bekor qilindi!";
          this.banDeviceAndUser(this.flagReason);
          return false;
        }
      } else {
      ...
      // Real-time instantaneous WPM ceiling check (Max 280 WPM human threshold)
      if (typedLength > 12 && this.testStartTime > 0) {
        const elapsedMinutes = (now - this.testStartTime) / 60000;
        if (elapsedMinutes > 0.02) {
          const instantWpm = (typedLength / 5) / elapsedMinutes;
          if (instantWpm > 280) {
            this.isFlagged = true;
            this.flagReason = `Insoniy rekorddan yuqori tezlik (${Math.round(instantWpm)} WPM > 280) aniqlandi va bekor qilindi!`;
            this.banDeviceAndUser(this.flagReason);
            return false;
          }
        }
  ...
  public validateTypingResult(wpm: number, accuracy: number, durationSeconds: number, charCount: number): boolean {
    ...
    const maxPossibleChars = durationSeconds * 28; // 28 chars/sec = max ~336 WPM burst
